#!/usr/bin/env python3
"""Draw the profile README's stat graphics from the GitHub API.
...
Contribution graphics (stats / streak / year) count your commits across every
owned repository the token can see — including private ones — but only as
anonymous daily totals. Repository names never leave the fetch step and never
appear in any SVG. Language totals stay public-only so private work isn't
fingerprinted by stack mix.
...
Env:
  GITHUB_TOKEN  required (needs `repo` to include private commit counts)
  GH_LOGIN      user to summarise (default: ANDREWBTC707)
...
# Languages stay public-only on purpose: private repos are counted in the
# anonymous commit calendar, but never contribute to language bars.
LANGS_QUERY = """
query($login: String!) {
  user(login: $login) {
...
def _headers(token, login, content_type=None):
...
def graphql(token, login, query, variables=None):
    body = json.dumps({"query": query, "variables": variables or {}}).encode()
    req = urllib.request.Request(
        API, data=body, headers=_headers(token, login, "application/json"))
...
def rest_json(token, login, url):
    req = urllib.request.Request(url, headers=_headers(token, login))
    try:
        with urllib.request.urlopen(req, timeout=60) as r:
            return json.load(r), r.headers
    except urllib.error.HTTPError as e:
        # Empty / uninitialized repos return 409; treat as no commits.
        if e.code in (404, 409):
            return [], e.headers
        raise


def rest_paginate(token, login, url):
    while url:
        data, headers = rest_json(token, login, url)
        if not data:
            return
...
def owned_repo_names(token, login, since_iso):
    """Full names of owned non-fork repos pushed at or after since_iso."""
    names = []
    url = (f"{REST}/user/repos?affiliation=owner&per_page=100"
           f"&sort=pushed&direction=desc")
    for repo in rest_paginate(token, login, url):
        if repo.get("fork"):
            continue
        pushed = repo.get("pushed_at") or ""
        # Repos sorted by pushed_at desc — stop once we're outside the window.
...
def commit_day_counts(token, login, since_iso):
    """Anonymous UTC-day commit counts across every owned repo.

    Repo names are used only to page the API, then discarded. The returned
    Counter maps YYYY-MM-DD → commit count and is the only artifact kept.
    """
    counts = Counter()
    for full_name in owned_repo_names(token, login, since_iso):
        q = urllib.parse.urlencode({
            "author": login,
            "since": since_iso,
            "per_page": "100",
        })
        url = f"{REST}/repos/{full_name}/commits?{q}"
        try:
            for commit in rest_paginate(token, login, url):
                iso = ((commit.get("commit") or {}).get("author") or {}).get("date")
                if not iso:
                    continue
                counts[iso[:10]] += 1
...
def calendar_from_counts(counts):
    """Build a GraphQL-shaped week grid pinned to whole UTC days."""
    start, today = window()
    # Align the grid to Sunday like GitHub's contribution calendar.
    ...
    while day <= today:
        ...
        for weekday in range(7):
            if day < start or day > today:
                ...
                week.append({"contributionCount": counts.get(day.isoformat(), 0),
                             "date": day.isoformat(),
                             "weekday": weekday})
            day += timedelta(days=1)
        weeks.append(week)
    # Trim leading / trailing all-zero weeks outside the pinned window so the
    # sparkline width stays stable; keep any week that intersects [start, today].
    return weeks


def fetch_public_languages(token, login):
    data = graphql(token, login, LANGS_QUERY, {"login": login})
    user = data.get("user")
    if not user:
...
def summarise(token, login):
    since, _until = window_iso()
    # Private + public commits, folded into day counts only — no repo names.
    counts = commit_day_counts(token, login, since)
    weeks = calendar_from_counts(counts)
    days = [d for w in weeks for d in w]
    # Only count days inside the pinned window for the hero total.
    start, today = window()
    in_window = [d for d in days
                 if start.isoformat() <= d["date"] <= today.isoformat()]
    weekly = [sum(d["contributionCount"] for d in w
                  if start.isoformat() <= d["date"] <= today.isoformat())
              for w in weeks]
    cur, best = streaks(in_window)
    by_size, by_repo = languages(fetch_public_languages(token, login))
    return dict(
        total=sum(d["contributionCount"] for d in in_window),
        active=sum(1 for d in in_window if d["contributionCount"] > 0),
...
def main():
    ...
    login = os.environ.get("GH_LOGIN", "ANDREWBTC707")
    ...
    print(f"{s['total']} commits (anon), {s['active']} active days, "
          f"best week {s['best_week']}, current streak "
          f"{s['current']['length']}, longest {s['longest']['length']}")
    print("languages by bytes (public only): "
          + (", ".join(f"{n} {v}" for n, v in s["by_size"]) or "none"))
